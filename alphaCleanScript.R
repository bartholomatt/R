...
interpretGrades = function(input){
  ...
  studentList = readxl::read_xlsx(input,sheet=sheets[1],col_names = TRUE) %>% 
    select(c(1,2)) %>% 
    slice(-1) %>% 
    rename(GradeLevel = ...2) %>% 
  ...
  gradesList = readxl::read_xlsx(input, sheet=sheets[2]) %>% 
  ...
  for(num in 2:length(sheets)){
    ...
    toAdd = readxl::read_xlsx(input, sheet=sheets[num]) %>% 
  ...
  toAdd = readxl::read_xlsx(input, sheet=sheets[4]) %>% 
    select(c(TermTested,Subject, StudentID, StartScore)) %>%  
    rename(Score = StartScore) %>% 
